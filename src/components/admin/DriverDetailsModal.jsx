...
const DriverDetailsModal = ({ driver, isOpen, onClose, onDriverUpdate }) => {
    ...
    useEffect(() => {
        const handleEscapeKey = (event) => {
            if (event.key === 'Escape' && isOpen) {
                ...
                try {
                    onClose();
                    console.log('🔒 onClose function called successfully from Escape key');
                } catch (error) {
                    console.error('🔒 Error calling onClose from Escape key:', error);
                }
            }
        };
    ...
    const handleBackdropClick = () => {
        ...
        try {
            onClose();
            console.log('🔒 onClose function called successfully from backdrop');
        } catch (error) {
    ...
    if (!driver) {
        console.log('🔒 DriverDetailsModal: No driver provided, not rendering');
        return null;
    }

    if (!isOpen) {
        console.log('🔒 DriverDetailsModal: Modal is not open, not rendering');
        return null;
    }

    console.log('🔒 DriverDetailsModal: Rendering modal for driver:', driver.name);
    ...
    return (
        <div className="fixed inset-0 z-[9999] overflow-hidden">
            ...
            {/* Modal Container */}
            <div className="absolute inset-0 flex items-center justify-center p-4" style={{ zIndex: 10001 }}>
                <div
                                        ...
                                        <div className="bg-gray-50 rounded-lg p-3">
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <div className="flex items-center justify-between text-xs">
                                                    ...
                                                    <span className={`px-2 py-1 rounded-full text-xs ${driver?.isOnline ? 'bg-green-300 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
